...
def requestSchoolLinePlus(schoolid, provinceName, stype):
    provinceid = GprovinceID[provinceName]
    url = "https://static-data.eol.cn/www/2.0/schoolprovinceindex/detial/%d/%d/%d/1.json" % (schoolid, provinceid, stype)
    headers = headers = {'Accept': 'text/plain, application/json, */*',
                         'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.122 Safari/537.36'}
    result = requests.get(url, headers=headers)
    info = json.loads(result.text)
    ...
    if count > 10:
        for i in range((int(int(count) / 10))):
            time.sleep(5)
            url = "https://static-data.eol.cn/www/2.0/schoolprovinceindex/detial/%d/%d/%d/%d.json" % (schoolid, provinceid, stype, i + 2)
            result = requests.get(url, headers=headers)
            info = json.loads(result.text)
            if info['code'] != "0000":
    ...
    for i in range(len(schoolLineList)):
        ...
        cursor = c.execute(
            "SELECT * from SCHOOLLINEPLUS where SCHOOLID=%d AND PROVINCE='%s' AND STYPE='%s' AND YEAR=%d AND MIN='%s' AND TYPE='%s' AND BATCH='%s'" % (schoolid, provinceName, stype, year, minScore, zslx, batch))
        result = cursor.fetchall()
        if len(result) == 0:
    ...
    for schoolName in schoolList:
        ...
        if schoolInfo :
            ...
            if (datetime.datetime.now().year - firstItem[4]) <= 1:
                ...
                tryToRequestSchoolLinePlus(schoolid, schoolName, provinceName, stype, failList)
        else:
...
def main():
    ...
    res = {}
    for t in result:
        cursor = c.execute("SELECT name from SCHOOLID where schoolid=%d" % (t[1]))
        result = cursor.fetchall()
        kname = result[0][0]
    ...
    for k, v in res.items():
        ...
        count = count + 1;
        if count % 10 == 0:
            plt.plot(baseyear, baseline, 'o', label='%s省本科线' %(provinceName), linestyle='-', color='black')
            plt.rcParams['font.sans-serif'] = ['Microsoft YaHei']
            plt.legend(bbox_to_anchor=[1, 1])
            plt.grid()
            plt.ylim((325, 550))
